...
# Configure Nginx for localhost access - SIMPLIFIED VERSION
configure_nginx_localhost() {
    log_info "Configuring Nginx for localhost/network access..."
    
    # Find cinestream directory
    local APP_DIR="$WWW_ROOT/cinestream"
    if [[ ! -d "$APP_DIR" ]] || [[ ! -f "$APP_DIR/.deploy_config" ]]; then
        log_error "CineStream not found at $APP_DIR"
        return 1
    fi
    
    # Load config
    source "$APP_DIR/.deploy_config"
    local START_PORT="${START_PORT:-8001}"
    local PROCESS_COUNT="${PROCESS_COUNT:-20}"
    
    log_info "Creating simple nginx configuration..."
    log_info "Backend: $PROCESS_COUNT instances starting at port $START_PORT"
    
    # Create nginx config
    cat > /etc/nginx/conf.d/cinestream.conf <<EOF
...
EOF
    ...
    for ((i=0; i<PROCESS_COUNT; i++)); do
        local instance_port=$((START_PORT + i))
        echo "    server 127.0.0.1:$instance_port;" >> /etc/nginx/conf.d/cinestream.conf
    done
...
EOF
    ...
    log_success "Nginx configuration created: /etc/nginx/conf.d/cinestream.conf"
    
    # Remove conflicting configs
    log_info "Removing conflicting default configurations..."
